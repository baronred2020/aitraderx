...
class EURUSDMultiStrategyAI:
    ...
    def save_all_models(self, filepath_prefix="eurusd_strategies"):
        ...
        models_dir = "models/trained_models/Brain_Ultra/EURUSD"
        ...
        for strategy_name, strategy in self.strategies.items():
            if strategy['is_trained']:
                ...
                for model_name, model in strategy['models'].items():
                    model_path = os.path.join(models_dir, f"{strategy_prefix}_{model_name}.pkl")
                    scaler_path = os.path.join(models_dir, f"{strategy_prefix}_{model_name}_scaler.pkl")
                ...
                metadata_path = os.path.join(models_dir, f"{strategy_prefix}_metadata.pkl")
                joblib.dump(metadata, metadata_path)
                
                print(f"✅ Estrategia {strategy_name} guardada en {models_dir}")
    ...
    def load_all_models(self, filepath_prefix="eurusd_strategies"):
        ...
        for strategy_name in self.strategies.keys():
            try:
                ...
                self.strategies[strategy_name]['is_trained'] = True
                print(f"✅ Estrategia {strategy_name} cargada desde {models_dir}")
                
            except FileNotFoundError:
                print(f"⚠️ No se encontraron modelos para {strategy_name} en {models_dir}")
            except Exception as e:
...
def auto_train_system(force_retrain=False):
    ...
    try:
        ...
        model_files = [
            os.path.join(models_dir, "eurusd_real_data_scalping_lightgbm.pkl"),
            os.path.join(models_dir, "eurusd_real_data_day_trading_lightgbm.pkl"),
            os.path.join(models_dir, "eurusd_real_data_swing_trading_lightgbm.pkl"),
            os.path.join(models_dir, "eurusd_real_data_position_trading_lightgbm.pkl")
        ]
