        ...
        def objective(trial):
            try:
                ...
                if model_type != 'LSTM':
                    ...
                    try:
                        # Verificar que los datos no tienen NaN o infinitos (sin warning)
                        ...
                        # Cross-validation con manejo robusto de NaN
                        try:
                            cv_scores = cross_val_score(model, X, y, cv=cv, scoring='accuracy', n_jobs=1)
                            
                            # Filtrar scores NaN y usar solo valores válidos
                            valid_scores = cv_scores[~np.isnan(cv_scores)]
                            
                            if len(valid_scores) == 0:
                                # Si no hay scores válidos, usar score por defecto
                                return 0.5
                            
                            # Usar solo scores válidos para calcular la media
                            mean_score = valid_scores.mean()
                            
                            # Verificar que el score final es válido
                            if np.isnan(mean_score) or mean_score < 0 or mean_score > 1:
                                return 0.5
                                
                        except Exception as cv_error:
                            # Si hay error en CV, usar score por defecto
                            return 0.5
