...
class ForexForecastingSystem:
    ...
    def save_models(self, base_dir: str = 'models/trained_models/Brain_Predictor'):
        """Guardar modelos entrenados organizados por símbolo y horizonte"""
        try:
            import os
            import json
            ...
            # Guardar modelos por símbolo y horizonte
            for model_key, model_data in self.models.items():
                # Parsear el modelo key (ej: "EURUSD=X_7d")
                symbol, horizon = model_key.split('_')
                
                # Crear directorio del símbolo
                symbol_dir = os.path.join(base_dir, symbol.replace('=X', ''))
                os.makedirs(symbol_dir, exist_ok=True)
                
                # Guardar modelo de forecasting
                forecasting_model_path = os.path.join(symbol_dir, f'forecasting_model_{horizon}.pkl')
                joblib.dump({
                    'price_model': model_data['price_model'],
                ...
                importance_path = os.path.join(symbol_dir, f'feature_importance_{horizon}.json')
                with open(importance_path, 'w') as f:
                    json.dump(self.feature_importance[model_key], f, indent=2)
                
                logger.info(f"✅ Modelo {model_key} guardado en {symbol_dir}")
            
            # Guardar metadata general del sistema
            metadata = {
            ...
            metadata_path = os.path.join(base_dir, 'system_metadata.json')
            with open(metadata_path, 'w') as f:
                json.dump(metadata, f, indent=2)
            
            logger.info(f"✅ Sistema de modelos guardado en {base_dir}")
            logger.info(f"   - Símbolos entrenados: {len(metadata['trained_symbols'])}")
            logger.info(f"   - Total modelos: {metadata['total_models']}")
            
        except Exception as e:
            logger.error(f"❌ Error guardando modelos: {e}")
    
    def load_models(self, base_dir: str = 'models/trained_models/Brain_Predictor'):
        """Cargar modelos entrenados desde la estructura organizada"""
        try:
            import os
            import json
            ...
            if os.path.exists(metadata_path):
                ...
                logger.info(f"📊 Cargando sistema con {metadata['total_models']} modelos")
            
            # Cargar modelos por símbolo
            for symbol_dir in os.listdir(base_dir):
                ...
                if os.path.isdir(symbol_path):
                    ...
                    for file in os.listdir(symbol_path):
                        if file.startswith('forecasting_model_') and file.endswith('.pkl'):
                            horizon = file.replace('forecasting_model_', '').replace('.pkl', '')
                            model_key = f"{symbol}_={horizon}"
                            
                            # Cargar modelo
                            model_path = os.path.join(symbol_path, file)
                            model_data = joblib.load(model_path)
                            
                            self.models[model_key] = model_data
                            
                            # Cargar feature importance si existe
                            importance_path = os.path.join(symbol_path, f'feature_importance_{horizon}.json')
                            if os.path.exists(importance_path):
                                with open(importance_path, 'r') as f:
                                    self.feature_importance[model_key] = json.load(f)
                            
                            logger.info(f"✅ Modelo {model_key} cargado")
            
            logger.info(f"✅ Sistema cargado: {len(self.models)} modelos")
            return True
    ...
    def run_intelligent_monitoring_schedule(self, symbols: List[str] = None):
        ...
        models_path = 'models/trained_models/Brain_Predictor'
        if self.load_models(models_path):
...
def main():
    ...
    if args.train:
        ...
        import os
        models_dir = 'models/trained_models/Brain_Predictor'
        os.makedirs(models_dir, exist_ok=True)
        forecasting_system.save_models(models_dir)
