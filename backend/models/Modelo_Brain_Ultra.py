...
def get_real_market_data(symbol, timeframe, periods):
    """
    ...
    """
    print(f"📊 Descargando datos {symbol} reales: {timeframe} - {periods}")
...
def create_strategy_datasets_real(symbol='EURUSD=X'):
    ...
    for strategy, config in strategy_configs.items():
        ...
        data = get_real_market_data(symbol, config['timeframe'], config['period'])
...
def generate_market_data(symbol, timeframe, periods):
    """Genera datos sintéticos para cualquier símbolo"""
    # Usar la función EURUSD como base y ajustar según el símbolo
    data = generate_eurusd_data(timeframe, periods)
    ...
    data['close'] *= price_ratio
    
    return data

class UniversalMultiStrategyAI:
    """
    Sistema de IA universal para cualquier símbolo con 4 estrategias de trading:
    - Scalping (1M, 5M)
    - Day Trading (15M, 1H) 
    - Swing Trading (4H, 1D)
    - Position Trading (1D, 1W)
    ...
    def __init__(self, symbol='EURUSD=X'):
        ...
        if symbol == 'USDJPY=X':
            ...
            confidence_thresholds = {
                ...
                'day_trading': 60,
                'swing_trading': 20,  # Reducido para más señales BUY
                'position_trading': 70
            }
        else:
            # Thresholds estándar para otros pares
            confidence_thresholds = {
                'scalping': 75,
                'day_trading': 70,
                'swing_trading': 65,
        ...
        self.strategies = {
            'scalping': {
                'timeframes': ['1T', '5T'],
                'target_pips': int(2 * self.symbol_config['volatility_factor']),
                'stop_loss_pips': int(1 * self.symbol_config['volatility_factor']),
                'confidence_threshold': confidence_thresholds['scalping'],
                'models': {}, 'scalers': {}, 'is_trained': False
            },
            'day_trading': {
                'timeframes': ['15T', '1H'],
                'target_pips': int(15 * self.symbol_config['volatility_factor']),
                'stop_loss_pips': int(8 * self.symbol_config['volatility_factor']),
                'confidence_threshold': confidence_thresholds['day_trading'],
                'models': {}, 'scalers': {}, 'is_trained': False
            },
            'swing_trading': {
                'timeframes': ['4H', '1D'],
                'target_pips': int(100 * self.symbol_config['volatility_factor']),
                'stop_loss_pips': int(50 * self.symbol_config['volatility_factor']),
                'confidence_threshold': confidence_thresholds['swing_trading'],
                'models': {}, 'scalers': {}, 'is_trained': False
            },
            'position_trading': {
                'timeframes': ['1D', '1W'],
                'target_pips': int(500 * self.symbol_config['volatility_factor']),
                'stop_loss_pips': int(200 * self.symbol_config['volatility_factor']),
                'confidence_threshold': confidence_thresholds['position_trading'],
                'models': {}, 'scalers': {}, 'is_trained': False
            }
        }
        
        # Directorio específico para el símbolo
        self.models_dir = f"models/trained_models/Brain_Ultra/{symbol.replace('=X', '')}"
        import os
        os.makedirs(self.models_dir, exist_ok=True)
        
        print(f"[INFO] Sistema inicializado para {symbol}")
        print(f"[INFO] Pip value: {self.pip_value}")
        print(f"[INFO] Volatility factor: {self.symbol_config['volatility_factor']}")
    
    # Heredar todos los métodos de EURUSDMultiStrategyAI
    def _initialize_models_for_strategy(self, strategy_name):
        """Inicializa modelos específicos para cada estrategia"""
    ...
    def train_strategy(self, data, strategy_name, validation_split=0.2):
        ...
        for model_name, model in self.strategies[strategy_name]['models'].items():
            ...
            try:
                ...
                # Convertir a pips para Forex
                val_pips = val_mae / self.pip_value
    ...
    def generate_signals_strategy(self, data, strategy_name):
        ...
        if self.symbol == 'USDJPY=X':
            ...
            if strategy_name == 'swing_trading':
                ...
                pip_threshold = strategy_config['target_pips'] * self.pip_value * 0.01  # Más sensible para GBPUSD
        else:
            pip_threshold = strategy_config['target_pips'] * self.pip_value * 0.1
        confidence_threshold = strategy_config['confidence_threshold']
        ...
        for i, pred in enumerate(ensemble_pred):
            ...
            if signal == 'BUY':
                take_profit = current_price + (strategy_config['target_pips'] * self.pip_value)
                stop_loss = current_price - (strategy_config['stop_loss_pips'] * self.pip_value)
            elif signal == 'SELL':
                take_profit = current_price - (strategy_config['target_pips'] * self.pip_value)
                stop_loss = current_price + (strategy_config['stop_loss_pips'] * self.pip_value)
            else:
    ...
    def train_all_strategies(self, data_dict):
        """Entrena todas las estrategias con sus respectivos datos"""
        print(f"🔥 ENTRENANDO TODAS LAS ESTRATEGIAS {self.symbol}")
        print("=" * 60)
    ...
    def save_all_models(self, filepath_prefix=None):
        """Guarda todos los modelos entrenados"""
        if filepath_prefix is None:
            filepath_prefix = f"{self.symbol.replace('=X', '')}_strategies"
        
        # Usar el directorio específico del símbolo
        import os
        symbol_dir = f"{self.symbol.replace('=X', '')}"
        save_dir = os.path.join(self.models_dir, symbol_dir)
        ...
        for strategy_name, strategy in self.strategies.items():
            if strategy['is_trained']:
                ...
                for model_name, model in strategy['models'].items():
                    model_path = os.path.join(save_dir, f"{strategy_prefix}_{model_name}.pkl")
                    scaler_path = os.path.join(save_dir, f"{strategy_prefix}_{model_name}_scaler.pkl")
                ...
                metadata_path = os.path.join(save_dir, f"{strategy_prefix}_metadata.pkl")
                joblib.dump(metadata, metadata_path)
                
                print(f"✅ Estrategia {strategy_name} guardada en {save_dir}")
    
    def load_all_models(self, filepath_prefix=None):
        """Carga todos los modelos guardados"""
        if filepath_prefix is None:
            filepath_prefix = f"{self.symbol.replace('=X', '')}_strategies"
        ...
        import os
        symbol_dir = f"{self.symbol.replace('=X', '')}"
        load_dir = os.path.join(self.models_dir, symbol_dir)
        ...
        for strategy_name in self.strategies.keys():
            try:
                ...
                # Cargar metadatos
                metadata_path = os.path.join(load_dir, f"{strategy_prefix}_metadata.pkl")
                ...
                # Cargar modelos y escaladores
                for model_name in self.strategies[strategy_name]['models'].keys():
                    model_path = os.path.join(load_dir, f"{strategy_prefix}_{model_name}.pkl")
                    scaler_path = os.path.join(load_dir, f"{strategy_prefix}_{model_name}_scaler.pkl")
                    
                    self.strategies[strategy_name]['models'][model_name] = joblib.load(model_path)
                    self.strategies[strategy_name]['scalers'][model_name] = joblib.load(scaler_path)
                
                self.strategies[strategy_name]['is_trained'] = True
                print(f"✅ Estrategia {strategy_name} cargada desde {load_dir}")
                
            except FileNotFoundError:
                print(f"⚠️ No se encontraron modelos para {strategy_name} en {load_dir}")
            except Exception as e:
                print(f"❌ Error cargando {strategy_name}: {e}")
...
def main_universal_trading(symbol='EURUSD=X'):
    ...
    try:
        ...
        datasets = create_strategy_datasets_real(symbol)
        ...
        # 2. Inicializar el sistema multi-estrategia universal
        print("\n2️⃣ Inicializando sistema multi-estrategia universal...")
        ai_system = UniversalMultiStrategyAI(symbol)
        
        # 3. Entrenar todas las estrategias
        print("\n3️⃣ Entrenando todas las estrategias...")
        all_results = ai_system.train_all_strategies(datasets)
        ...
        all_signals = ai_system.get_all_signals(recent_datasets)
        ...
        for cycle in range(1, 4):
            ...
            # Obtener señales
            live_signals = ai_system.get_all_signals(new_data)
        ...
        print(f"   - {symbol.replace('=X', '')}_strategies_*.pkl")
        print("\n💡 Próximos pasos:")
...
if __name__ == "__main__":
    ...
    # Verificar si se proporcionó un símbolo como argumento
    if len(sys.argv) > 1:
        symbol = sys.argv[1]
        print(f"🚀 ENTRENANDO {symbol} DESDE LÍNEA DE COMANDOS")
        print("=" * 60)
        
        # Entrenar el símbolo especificado
        ai_system = main_universal_trading(symbol)
        
        if ai_system:
            print(f"\n✅ {symbol} entrenado exitosamente!")
        else:
            print(f"\n❌ Error entrenando {symbol}")
    else:
        # Ejecutar sistema completo (EURUSD por defecto)
        print("🚀 EJECUTANDO SISTEMA COMPLETO (EURUSD)")
        print("=" * 60)
        ...
        print("python train_any_pair.py EURUSD")
